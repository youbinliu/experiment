			...
			<div class="span9"> 
				...
				<?php if(!empty($vms_list)):
					foreach ($vms_list as $id=>$vm){
						...
						echo '<i class="icon-circle-arrow-down"></i><a href="'?><?php echo base_url("/resource/saveimage/".$id);?><?php echo '" data-confirm="此操作会将该虚拟机作为实验环境保存<br>稍后可以使用该镜像创建虚拟机！<br>操作同时会<span style=color:red>删除</span>虚拟机！<br>确定执行？"> 保存实验环境并关闭 </a>';
